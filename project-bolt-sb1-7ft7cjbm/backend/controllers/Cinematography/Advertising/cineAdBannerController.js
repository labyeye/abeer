const AdBanner = require('../../../models/Cinematography/Advertising/CineAdBanner');
...
exports.getActiveBanner = async (req, res) => {
  try {
    const banner = await AdBanner.findOne({ isActive: true });
    if (!banner) {
...
exports.updateBanner = async (req, res) => {
  try {
    ...
    if (isActive) {
      await AdBanner.updateMany({ _id: { $ne: id } }, { isActive: false });
    }

    const updatedBanner = await AdBanner.findByIdAndUpdate(id, updateData, { new: true });
