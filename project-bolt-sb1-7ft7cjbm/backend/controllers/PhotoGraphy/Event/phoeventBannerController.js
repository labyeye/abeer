...
exports.getActiveBanner = async (req, res) => {
  try {
    const banner = await EventBanner.findOne({ isActive: true });
    if (!banner) {
...
exports.updateBanner = async (req, res) => {
  try {
    ...
    if (isActive) {
      await EventBanner.updateMany({ _id: { $ne: id } }, { isActive: false });
    }

    const updatedBanner = await EventBanner.findByIdAndUpdate(id, updateData, { new: true });
...
exports.deleteBanner = async (req, res) => {
  try {
    await EventBanner.findByIdAndDelete(req.params.id);
