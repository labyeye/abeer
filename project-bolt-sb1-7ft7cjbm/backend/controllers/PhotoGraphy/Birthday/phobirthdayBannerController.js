...
exports.getActiveBanner = async (req, res) => {
  try {
    const banner = await BirthdayBanner.findOne({ isActive: true });
    if (!banner) {
...
exports.getAllBanners = async (req, res) => {
  try {
    const banners = await BirthdayBanner.find().sort({ createdAt: -1 });
    res.json(banners);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
exports.createBanner = async (req, res) => {
  try {
    ...
    const newBanner = new BirthdayBanner({
      title,
      description,
      backgroundImageUrl,
...
exports.updateBanner = async (req, res) => {
  try {
    ...
    if (isActive) {
      await BirthdayBanner.updateMany({ _id: { $ne: id } }, { isActive: false });
    }

    const updatedBanner = await BirthdayBanner.findByIdAndUpdate(id, updateData, { new: true });
...
exports.deleteBanner = async (req, res) => {
  try {
    await BirthdayBanner.findByIdAndDelete(req.params.id);
