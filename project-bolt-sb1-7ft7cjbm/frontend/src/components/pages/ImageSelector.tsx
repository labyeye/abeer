...
const ImageSelector = () => {
  ...
  const fetchRootFolders = async () => {
    ...
    try {
      const response = await axios.get(
        "https://abeer.onrender.com/api/folders/roots"
      );
  ...
  const fetchSubFolders = async (folderId: string) => {
    ...
    try {
      const response = await axios.get(
        `https://abeer.onrender.com/api/folders/${folderId}/subfolders`
      );
  ...
  const fetchFolderImages = async (folderId: string) => {
    ...
    try {
      const response = await axios.get(
        `https://abeer.onrender.com/api/folders/${folderId}/contents`
      );
    ...
    <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "20px" }}>
      ...
      {selectedRootFolder && (selectedSubFolder || subFolders.length === 0) && (
        <>
          <Card
            title={`Images in ${
              selectedSubFolder?.name || selectedRootFolder.name
            }`}
