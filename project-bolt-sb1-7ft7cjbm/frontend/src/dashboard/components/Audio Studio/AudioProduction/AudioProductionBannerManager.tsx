...
const AudioProductionBannerManager = () => {
  ...
  const fetchBanners = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:2500/api/audio-production-banner",
        {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await axios.put(
          `http://localhost:2500/api/audio-production-banner/${editingId}`,
          formData,
          {
            headers: getAuthHeaders(),
          }
  ...
  const handleDelete = async (id: string) => {
    ...
    if (window.confirm("Are you sure you want to delete this banner?")) {
      try {
        await axios.delete(
          `http://localhost:2500/api/audio-production-banner/${id}`,
          {
            headers: getAuthHeaders(),
          }
        );
