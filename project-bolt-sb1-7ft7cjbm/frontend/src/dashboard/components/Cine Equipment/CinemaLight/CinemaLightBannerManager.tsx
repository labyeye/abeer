...
const CinemaLightBannerManager = () => {
  ...
  const fetchBanners = async () => {
    ...
    try {
      const response = await axios.get(
        "https://abeer.onrender.com/api/cineequip-light-banner",
        {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await axios.put(
          `https://abeer.onrender.com/api/cineequip-light-banner/${editingId}`,
          formData,
          {
            headers: getAuthHeaders(),
          }
  ...
  const handleDelete = async (id: string) => {
    ...
    if (window.confirm("Are you sure you want to delete this banner?")) {
      try {
        await axios.delete(
          `https://abeer.onrender.com/api/cineequip-light-banner/${id}`,
          {
            headers: getAuthHeaders(),
          }
        );
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Cinema Light Banner Manager</h1>
        <button
          onClick={() => {
            localStorage.removeItem("authToken");
            setAuthToken("");
...
export default CinemaLightBannerManager;
